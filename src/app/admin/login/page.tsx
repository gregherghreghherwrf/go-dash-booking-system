...
// MUST use the env var — never hardcode localhost (breaks on mobile/production)
  ...
  const login = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post(`${API}/api/admin/login`, {
        // Trim + lowercase — prevents mobile autocapitalize/autocorrect from breaking login
          ...
          <form onSubmit={login} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              <input
                ...
                /* These 4 attrs are CRITICAL on mobile — prevents autocorrect mangling the email */
                autoCapitalize="none"
                autoCorrect="off"
                autoComplete="email"
                spellCheck={false}
